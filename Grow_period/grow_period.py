...
def aro_stage_ETL(root_path,grow_stage):
    ...
    for year in range(start_year, end_year):
        ...
        for stationID in year_grow_stage_data['AgoStationID']:
            ...
            row = str(stationID)+' '+str(lon)+' '+str(lat)+' '+ str(year) +' '+grow_stage + ' '+ str(Common_func.yyyymmdd_to_day_num(grow_stage_day))
    ...
    for index, lonlatdata in station_list.iterrows():
        try:
            ...
            for i in range(2001,2014):
                ...
                row = str(StationID)+' '+ str(lon)+' '+str(lat)+' '+year+' '+ str(round(stage_start_day)) +' '+ str(round(stage_end_day))
                print(row)
                Modis_IO.write_txt(result_file,row)
        except:
            print('no data')
